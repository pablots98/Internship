function [clustObj,Z,Hg,Tg,outpermg] = geneExprDist_hierarchy_add_bins(exprData,removeObjects,bins,k,distMethod,linkageMethod,shift)
% USAGE: 
...
% % shift           value to add to shift all of the expressed genes of the
                    %of the same value so to keep the zero expr genes in
                    %the clustering and thresholding %to check if it makes
                    %sense
...
%this updated (by LD) version of the function
   %1. add an input, "shift", that is the value that we want to use to shift
   %the expression of the genes to allow the inclusion of the "0" values
   %for enzymes that are not expressed in every tissue
   %2. keeps track of the 0-data enzymes and missing-data enzymes in new
   %elements of the structure to use them further on in the calculation of
   %the ubiScore.
   %3. keeps track of the 0 indices for the partially expressed genes, to
   %put them back to the zeroexpr ubiScore value in the ubiScore function
...
if  isfield(exprData,'gene')
    clustObj.zeroobjects = exprData.gene(sum(exprData.valuebyTissue,2)==0);
    exprData.gene(sum(exprData.valuebyTissue,2)==0) = [];
elseif isfield(exprData,'enzyme')
    enz=1;
    clustObj.zeroobjects = exprData.enzyme(sum(exprData.valuebyTissue,2)==0);
    clustObj.zerorxns = exprData.rxns(sum(exprData.valuebyTissue,2)==0);
...
[zerorow,zerocol]=find(exprData.valuebyTissue==0);

if ~isempty(shift)
    exprData.valuebyTissue = exprData.valuebyTissue + shift;
end
...
%bins calculations
dataedge=v(v>-inf);
edgeX = linspace(min(min(dataedge)),max(max(dataedge)),bins+1);
edgeX = round(edgeX,1);
...
clustObj.Data = v;
clustObj.missingobjects = exprData.missingEnz;
clustObj.missingobjectMaps = exprData.missingRxns;
clustObj.partialzeroIdx=[zerorow,zerocol];
clustObj.edgeX=edgeX;
