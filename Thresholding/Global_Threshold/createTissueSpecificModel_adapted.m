function [tissueModel] = createTissueSpecificModel_adapted(model, options, funcModel, exRxnRemove, optionalParams)
...
elseif ~isfield(options,'solver')
    ...
    switch options.solver
        ...
        case 'mCADRE'
            ...
            if ~isfield(options,'tol'),options.tol=1e-8;end
        % ADDED CODE: mCADRE_wMetaboliteProtection has same default values
        % as mCADRE
        case 'mCADRE_wMetaboliteProtection'
            if ~isfield(options,'ubiquityScore') || ~isfield(options,'confidenceScores')
                error('One of the 2 required option fields for mCADRE method is not defined')                
            end
            if ~isfield(options,'protectedRxns'),options.protectedRxns={};end
...
switch options.solver
    ...
    % ADDED CODE
    case 'mCADRE_wMetaboliteProtection'
        tissueModel = mCADRE_wMetaboliteProtection(model, options.ubiquityScore, options.confidenceScores, options.protectedRxns, options.checkFunctionality, options.eta, options.tol, options.precursorMets);
    case 'fastCore'
